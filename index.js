...
// nestedFunction can access internal because internal exist inside the scope where it is calling
...
function animalNames(array) {
  ...
  let newArr = [];
  array.forEach(el => {
    newArr.push(`name: ${el.animal_name}, scientific: ${el.scientific_name}`)
...
function lowerCaseNames(array) {
  /*Your Code Here*/
  return array.map(el => el.animal_name.toLowerCase());
}
...
function lowPopulationAnimals(array) {
  /*Your Code Here*/
  let newArr = [];
  array.filter(el => el.population < 5 ? newArr.push(el) : null);
  return newArr;
}
